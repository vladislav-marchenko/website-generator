...
} as const satisfies TemplatesCategories
...
const parseTemplateFields = (templateName: string): TemplateData => {
  ...
  if (!templatesCategories.hasOwnProperty(templateName)) return results
  const template = templateName as TemplateName

  const categories = templatesCategories[template] as TemplateCategory[]
  for (const category of categories) {
    for (const { defaultValues, ...fieldData } of category.fields) {
...
const getTemplatesData = () => {
  ...
  const templateNames = Object.keys(templatesCategories)
